...
# -----------------------------
# Canonical RCA2 / reason set (kept for normalisation hints only)
# -----------------------------
...
_NORMALIZE_RCA2: Dict[str, str] = {
    ...
    "Incorrect Calculator Logic": "Incorrect Calculator",
...
_RULES: Dict[str, re.Pattern] = {
    ...
    "Knowledge Gap–Offshore": _rx([
        "rejected comment", "need to", "how to", "split", "pre", "age", "gmp",
    ]),
}
...
def maybe_fit_model(df: pd.DataFrame,
    ...
    # normalise but DO NOT restrict final label space to CANONICAL_REASONS only
    df_lab[rca2_col] = df_lab[rca2_col].apply(lambda v: _normalize_rca2(v) or str(v).strip())
...
def classify_text(text: Optional[str],
    ...
    """
    Rules → RCA2 → ML → Other. Returns a reason string.
    """
    # 1) Rules
    ...
    # 2) RCA2 direct (normalise common variants)
    r2 = _normalize_rca2(rca2)
    if r2:
        return r2
    if isinstance(rca2, str) and rca2.strip():
        # accept explicit RCA2 labels even if they’re not in CANONICAL_REASONS
        return rca2.strip()
    ...
    try:
        if model_bundle and text and isinstance(text, str):
            ...
            labels = set(model_bundle.get("labels") or [])
            if vec is not None and clf is not None:
                pred = clf.predict(vec.transform([text]))[0]
                # accept model labels learned from data, not just the canonical tuple
                if not labels or (pred in labels):
                    return str(pred)
    except Exception:
        pass
...
def get_or_fit_model(df_all: pd.DataFrame,
    ...
    if bundle is not None:
        return bundle
    return maybe_fit_model(df_all, text_col=text_col, rca2_col=rca2_col, min_rows=20)
