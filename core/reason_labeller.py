...
# ---------- RCA2 normalisation (accepts given RCA2s even if not in map) ----------
...
def _normalize_rca2(v: Optional[str]) -> Optional[str]:
    ...
    if not s:
        return None
    return _NORMALIZE_RCA2.get(s, s)

# ---------- Frozen rulebook (learned from your Aug'25 data) ----------
...
def _compile_regex(words: Iterable[str]) -> re.Pattern:
    ...
    if not pats:
        pats = [r"a^"]  # match nothing
    return re.compile("|".join(pats), flags=re.I)
...
def classify_text(text: Optional[str], rca2: Optional[str] = None) -> str:
    """
    Rule-based with RCA2 override.
    1) RCA2 present → normalise and return.
    ...
    if t:
        ...
        if scores:
            return sorted(scores.items(), key=lambda kv: (-kv[1], -len(_RULE_TERMS[kv[0]])))[0][0]
