...
def _safe_month(dt: pd.Timestamp) -> pd.Timestamp:
    """Return first day of the month for dt."""
...
def parse_months_from_text(text: str, default_start: pd.Timestamp, default_end: pd.Timestamp):
    """Parse 'Jun 2025 to Aug 2025' etc. Return (start_month, end_month)."""
...
def parse_portfolio_from_text(text: str) -> str | None:
    # crude but effective: look for 'portfolio <name>' or standalone capitalized names we know
    # we'll also respect the 'portfolio:' param if semantic router extracted it
...
# Default months from cases, if available
if store["cases"].empty:
    # defensive: pick today
...
# Extract params: months + portfolio
