...
def _add_caption(slide, text: str, left_in, top_in, width_in):
    ...
    cap.text = text; cap.paragraphs[0].font.size = Pt(11); cap.paragraphs[0].font.color.rgb = RGBColor(90,90,90)

def _ppt_add_fig(slide, fig, left_in, top_in, width_in, caption=""):
    buf = io.BytesIO(); fig.savefig(buf, format="png", dpi=220, bbox_inches="tight"); buf.seek(0)
    slide.shapes.add_picture(buf, Inches(left_in), Inches(top_in), width=Inches(width_in))
    if caption: _add_caption(slide, caption, left_in, top_in + 2.85, width_in)
    return top_in + 3.15

def _df_to_table(slide, df: pd.DataFrame, left_in, top_in, width_in, max_rows=8, caption=""):
    df = df.head(max_rows); rows, cols = len(df.index)+1, len(df.columns)
    shape = slide.shapes.add_table(rows, cols, Inches(left_in), Inches(top_in), Inches(width_in), Inches(1.0)); table = shape.table
    for j, col in enumerate(df.columns):
        cell = table.cell(0, j); cell.text = str(col)
        run = cell.text_frame.paragraphs[0].runs[0]; run.font.bold = True; run.font.size = Pt(10); run.font.color.rgb = RGBColor(11,61,145)
        cell.fill.solid(); cell.fill.fore_color.rgb = RGBColor(240,245,255)
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        for j, val in enumerate(row):
            cell = table.cell(i, j); cell.text = "" if pd.isna(val) else str(val); cell.text_frame.paragraphs[0].font.size = Pt(9)
        if i % 2 == 0:
            for j in range(cols):
                c = table.cell(i, j); c.fill.solid(); c.fill.fore_color.rgb = RGBColor(250,250,250)
    for j in range(cols): table.columns[j].width = Inches(width_in/cols)
    if caption: _add_caption(slide, caption, left_in, top_in - 0.28, width_in)
    return top_in + 1.25
...
def _build_one_pager(title: str, subtitle: str | None,
    ...
    left_col, right_col, col_w, y_left, y_right = 0.5, 5.2, 4.4, 1.6, 1.6
    def place_block(which, caption, payload):
        nonlocal y_left, y_right
        left = (y_left <= y_right) or not two_column; x = left_col if left or not two_column else right_col; y = y_left if left or not two_column else y_right
        new_y = _ppt_add_fig(slide, payload, x, y, col_w, caption) if which=="fig" else _df_to_table(slide, payload, x, y+0.25, col_w, caption)
        if two_column: y_left, y_right = (new_y+0.15, y_right) if left else (y_left, new_y+0.15)
        else: y_left = new_y+0.15
    for cap, fig in (figs or []): place_block("fig", cap, fig)
    for cap, df in (tables or []): place_block("tbl", cap, df)
    out = io.BytesIO(); prs.save(out); out.seek(0); return out.read()

# ---------- NEW: Snapshot content helper ----------
def _get_snapshot_content(slug: str, store, params: Dict[str, Any]):
    """
    Ask each question module for snapshot-ready content.
    Returns dict with keys: title, subtitle, figs, tables
    """
    for prefix in QUESTION_MODULE_PREFIXES:
        try:
            mod = importlib.import_module(f"{prefix}.{slug}")
            if hasattr(mod, "build_snapshot"):
                ...
                return {
                    ...
                    "subtitle": "",
                    "figs": [],
                    "tables": [("Data", df)] if isinstance(df, pd.DataFrame) and not df.empty else [],
                }
        except ModuleNotFoundError:
            continue
        except Exception as e:
            st.error(f"Snapshot content error in {slug}: {e}")
    return {"title": slug, "subtitle": "", "figs": [], "tables": []}
...
def _get_graph_token(tenant_id: str, client_id: str, client_secret: str) -> str:
    ...
    if resp.status_code != 200: raise RuntimeError(f"Token request failed: {resp.status_code} {resp.text}")
...
def _send_via_graph(to_addrs: List[str], subject: str, html_body: str,
    ...
    tenant_id, client_id, client_secret, mailbox = cfg.get("tenant_id"), cfg.get("client_id"), cfg.get("client_secret"), cfg.get("mailbox")
...
with st.expander("✉️  Share / Email snapshot"):
    ...
    with col_a: fmt = st.selectbox("Format", ["PPTX (one page)"], index=0)
    with col_b: include_tables = st.checkbox("Include small summary tables", value=True)
    if st.button("Send snapshot"):
        to_list = list(selected); 
        if custom and "@" in custom: to_list.append(custom)
        if not to_list:
            ...
            try:
                ...
                _send_via_graph(to_addrs=to_list, subject=title,
                                html_body=f"<p>Hi,</p><p>Attached is the snapshot from <b>Halo Quality</b> ({slug}).</p>",
                                attachment_name=f"{slug}_snapshot.pptx", attachment_bytes=ppt_bytes)
...
try:
    ...
    if isinstance(result, tuple) and len(result) in (1, 2):
        title = result[0]; subtitle = result[1] if len(result) == 2 else None
        if isinstance(title, str) and title.strip(): st.subheader(title)
