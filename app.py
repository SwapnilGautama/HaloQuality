...
st.markdown(
    ...
    <style>
      ...
      .email-card{ border:1px solid #E9EDF4; border-radius:8px; padding:12px; background:#FAFBFF; }
    </style>
    """,
...
def _ppt_add_title(slide, title: str, subtitle: str | None, logo_bytes: bytes | None = None):
    ...
    run2.text = (subtitle or "Auto summary") + f"  •  {stamp}"
...
def _df_to_table(slide, df: pd.DataFrame, left_in, top_in, width_in, max_rows=8, caption=""):
    ...
    for j, col in enumerate(df.columns):
        ...
        cell.text_frame.paragraphs[0].runs[0].font.color.rgb = RGBColor(11, 61, 145)
    ...
    for i, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        if i % 2 == 0:
            for j in range(cols):
                c = table.cell(i, j)
                c.fill.solid(); c.fill.fore_color.rgb = RGBColor(250, 250, 250)
...
def _get_snapshot_content(slug: str, store, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with figs/tables **always** lists of (caption, payload).
    """
    for prefix in QUESTION_MODULE_PREFIXES:
        try:
            mod = importlib.import_module(f"{prefix}.{slug}")
            ...
            # Fallback: use run()’s dataframe if any
            if hasattr(mod, "run"):
...
def _build_one_pager(title: str, subtitle: str | None,
    ...
    def place_block(which: str, caption: str, payload):
        ...
        if two_column:
            if left: y_left = new_y + 0.15
            else:    y_right = new_y + 0.15
        else:
            y_left = new_y + 0.15
...
def _send_via_graph(to_addrs: List[str], subject: str, html_body: str,
    ...
    tenant_id = graph.get("tenant_id") or os.getenv("TENANT_ID")
    client_id = graph.get("client_id") or os.getenv("CLIENT_ID")
    client_secret = graph.get("client_secret") or os.getenv("CLIENT_SECRET")
    mailbox = graph.get("mailbox") or os.getenv("MAILBOX")
...
# ============== Share / Email snapshot (enabled for NPS, FPA, Complaints) =============
SNAPSHOT_ENABLED = {"nps_by_portfolio", "first_pass_accuracy", "complaints_june_by_portfolio"}
if slug in SNAPSHOT_ENABLED:
    with st.expander("📄 Share / Email snapshot", expanded=False):
        # recipients drop-down list – from secrets (optional convenience list)
        prelist = st.secrets.get("share", {}).get("address_book", [])
        selected = st.multiselect("Choose recipient(s)", options=prelist, default=[], placeholder="No options to select")
        extra = st.text_input("Or add another recipient (optional)", placeholder="name@company.com")
        fmt = st.selectbox("Format", ["PPTX (one page)"], index=0)
        include_summaries = st.checkbox("Include small summary tables", value=True)

        if st.button("Send snapshot"):
            try:
                rcpts = [x for x in selected if x] + ([extra] if extra else [])
                if not rcpts:
                    st.warning("Please add at least one recipient."); st.stop()

                content = _get_snapshot_content(slug, store, params)
                if include_summaries and not content["tables"]:
                    # nothing to add – keep going; figs alone are fine
                    pass
                ...
                subj = content["title"]
                body = f"<p>Attached is the latest one-page snapshot for <b>{slug}</b> from Halo Quality.</p>"
                _send_via_graph(
                    rcpts, subj, body,
                    attachments=[(f"{slug}_snapshot_{datetime.now().strftime('%Y%m%d_%H%M')}.pptx", ppt_bytes)]
                )
                st.success("Snapshot sent.")
            except Exception as e:
...
if isinstance(result, str):
    ...
    # If your modules return a (header, subheader) tuple
    for i, part in enumerate(result):
        if isinstance(part, str) and i == 0:
            st.markdown(part)
