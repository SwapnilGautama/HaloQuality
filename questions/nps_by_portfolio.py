...
def _prepare_surveys(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if month_col:
        # Try robust parsing (accepts many formats)
        # If it's already a date, fine; if it's like "Jun 2025", okay; if it's a month name, OK.
        parsed = pd.to_datetime(df[month_col], errors="coerce", dayfirst=True, infer_datetime_format=True)
        # If parse failed and looks like month names, try manual mapping with assumed day 1
        needs_fill = parsed.isna()
        if needs_fill.any():
            mm = (
                df.loc[needs_fill, month_col].astype(str).str.strip().str[:3].str.title()
            )
    ...
    if not nps_col:
        # fallback: try a generic 'score' column
    ...
    # If numeric mostly NA, maybe it's a text label (Promoter/Passive/Detractor)
...
def _aggregate_nps(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Returns:
      by_month_portfolio: index=(portfolio, _month) with NPS%, Promoters, Passives, Detractors, Total
      latest_snapshot: NPS by portfolio for the latest month available (wide)
    """
    if df.empty:
        return pd.DataFrame(), pd.DataFrame()

    # Filter rows with valid month
...
def run(store: Dict[str, Any], params: Dict[str, Any], user_text: Optional[str] = None):
    ...
    by_month_portfolio, latest_pivot = _aggregate_nps(df)
    ...
    if not by_month_portfolio.empty:
        # overall NPS across selected slice
        k = by_month_portfolio.copy()
        num = (k["NPS%"] * k["Total"]).sum(skipna=True)  # weight by counts
    ...
    # ----- Chart: NPS trend (selected portfolio or All) -----
    if not by_month_portfolio.empty:
        # If single portfolio, show one line; else show multiple lines
        fig, ax = plt.subplots()
        for p, g in by_month_portfolio.groupby("portfolio"):
            g = g.sort_values("_month")
            ax.plot(g["_month"].astype(str), g["NPS%"], marker="o", label=p)
        ax.set_xlabel("Month")
        ax.set_ylabel("NPS %")
        ax.set_title("NPS Trend")
        ax.legend(loc="best", fontsize=8)
        st.pyplot(fig, use_container_width=True)

    # ----- Data tables -----
    # 1) Latest month snapshot (wide by portfolio)
    if not latest_pivot.empty:
        st.markdown("#### Latest Month NPS (by Portfolio)")
        st.dataframe(latest_pivot.rename(columns={"NPS%": "NPS %"}), use_container_width=True)
