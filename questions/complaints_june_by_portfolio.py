...
def _reason_trend_df(comp: pd.DataFrame, portfolio: str, use_ai: bool) -> pd.DataFrame:
    ...
    out: Dict[str, List[int]] = { "RCA1": ["Delay","Procedure","Communication","System","Incorrect/Incomplete information"] }  # type: ignore
    for m, label in zip(months, lab_months):
        subset = comp.loc[(comp[port_k] == portfolio) & (comp["_month"] == m), [desc_col]]
        if subset.empty:
            out[label] = [0,0,0,0,0]  # type: ignore
            continue
        r1, _ = _rca_labels_for_subset(subset[desc_col], use_ai=use_ai)
...
def _rca_year_counts(comp: pd.DataFrame, use_ai: bool) -> pd.DataFrame:
    """Month × RCA1 × RCA2 counts for Jan–Aug 2025."""
...
def _delay_attribution_share(comp: pd.DataFrame, use_ai: bool) -> Tuple[float, float]:
    """Return (External %, Aptia %) within Delay for Jan–Aug 2025."""
    _, _, date_k, desc_col = _detect_complaints_fields(comp)
    if any(x is None for x in [date_k, desc_col]):
...
def _combo_predictions(comp: pd.DataFrame, cases: pd.DataFrame, use_ai: bool) -> pd.DataFrame:
    """Top 5 Portfolio×RCA2 by complaints in 2025 (Jan–Aug), with each portfolio's avg Per1000."""
...
# -------------------------------
# Additional overall computations (cached)
# -------------------------------
...
# Optional quick import check if run directly (safe to leave or remove)
